...
class TestDataBaseInterface:
    ...
    def test_get_project_by_name(self, mock_connect, mock_disconnect, db_interface):
        ...
        db_interface.connect.assert_called_once()
        cursor.execute.assert_called_once_with(
            '''SELECT id FROM projects where owner_id = %s AND name = %s;''', (1, project_name))
        cursor.fetchone.assert_called_once()
        db_interface.disconnect.assert_called_once_with(mysql)
    ...
    def test_get_user_by_mail(self, mock_connect, mock_disconnect, db_interface):
        ...
        db_interface.connect.assert_called_once()
        cursor.execute.assert_called_once_with(
            '''SELECT id, name, email, password FROM users WHERE email = %s''', ('test_email',))
        cursor.fetchone.assert_called_once()
        db_interface.disconnect.assert_called_once_with(mysql)
    ...
    def test_get_stages(self, mock_connect, mock_disconnect, db_interface):
        ...
        db_interface.connect.assert_called_once()
        cursor.execute.assert_called_once_with(
            '''SELECT stages.id, stage_name, project_id, last_updated FROM ((stages LEFT JOIN projects ON projects.id = stages.project_id) LEFT JOIN users ON projects.owner_id = users.id) WHERE projects.id = %s AND users.id = %s ORDER BY stages.id ASC;''', (1, 1))
        cursor.fetchall.assert_called_once()
        db_interface.disconnect.assert_called_once_with(mysql)
    ...
    def test_get_stage(self, mock_connect, mock_disconnect, db_interface):
        ...
        db_interface.connect.assert_called_once()
        cursor.execute.assert_called_once_with(
            '''SELECT stages.id, stage_name, project_id, days, seconds, price, last_updated  FROM ((stages LEFT JOIN projects ON projects.id = stages.project_id) LEFT JOIN users ON projects.owner_id = users.id) WHERE stages.id = %s AND projects.id = %s and users.id = %s ;''', (1, 1, 1))
        cursor.fetchone.assert_called_once()
        db_interface.disconnect.assert_called_once_with(mysql)
