...
class DatabaseInterface:
    ...
    def insert_stage(self, stage_name, project_id, days, seconds, price, last_updated):
        self.__cursor().execute(
            """INSERT INTO stages (name, project_id, days, seconds, price, last_updated) VALUES (%s, %s, %s, %s, %s, %s)""",
    ...
    def get_stage(self, project_id, stage_id, user_id):
        cursor = self.__cursor()
        cursor.execute(
            """SELECT stages.id, stages.name, project_id, days, seconds, price, last_updated  FROM ((stages LEFT JOIN projects ON projects.id = stages.project_id) LEFT JOIN users ON projects.owner_id = users.id) WHERE stages.id = %s AND projects.id = %s and users.id = %s ;""",
            (stage_id, project_id, user_id),
        )
    ...
    def get_stage_by_name(self, stage_name, project_id, user_id):
        cursor = self.__cursor()
        cursor.execute(
            """SELECT stages.id FROM ((stages LEFT JOIN projects ON projects.id = stages.project_id) LEFT JOIN users ON projects.owner_id = users.id) WHERE stages.name = %s AND projects.id = %s and users.id = %s ;""",
            (stage_name, project_id, user_id),
        )
        stage = cursor.fetchone()
        return stage
